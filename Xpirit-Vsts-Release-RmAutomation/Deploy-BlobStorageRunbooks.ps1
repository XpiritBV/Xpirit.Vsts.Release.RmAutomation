...
[System.Collections.ArrayList] $blobs = Get-AzureStorageBlob -Container $StorageContainerName -Context $SourceContext

if ($blobs)   
{
	for ( $i = 20; $i -ge 0;  $i--)
    {
	    $blobs | Where-Object { $_.Name.Split('/').Length -eq $i -AND $_.Name.EndsWith(".ps1")} | Invoke-Parallel -ImportVariables -ImportModules -ScriptBlock {
		  	$Name = $_.Name.Split('/')[$_.Name.Split('/').Length-1].Split('.')[0].ToString()
			$RunbookURI = -join($SourceContext.BlobEndPoint, $StorageContainerName,"/", $_.Name)

			Write-Output "Deploying runbook $Name"	

			New-AzureRmResourceGroupDeployment -Name ((Get-ChildItem $TemplateFile).BaseName + '-' + $Name.SubString(0,[math]::min( $Name.Length , 30 )) + '-' + ((Get-Date).ToUniversalTime()).ToString('MMdd-HHmm')) `
