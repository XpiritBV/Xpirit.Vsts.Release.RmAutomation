...
Foreach ($file in $VariableFilesSplit){
    if ($file){
		...
		$Variables.variables | Invoke-Parallel -ImportVariables -ImportModules -ScriptBlock {
		    Write-Output "Deploying $_"
			New-AzureRmResourceGroupDeployment -Name ((Get-ChildItem $TemplateFile).BaseName + '-'+ $_.name.Replace(" ","") + '-' + ((Get-Date).ToUniversalTime()).ToString('MMdd-HHmm')) `
									-ResourceGroupName $ResourceGroupName `
									-TemplateFile $TemplateFile -TemplateParameterObject  @{accountName=$AutomationAccount;variableName=$_.name;variableType="string";variableValue=$_.value} -Force -Verbose | Out-Null
		    Write-Output "Variable $_ deployed"
		}
	}
}
