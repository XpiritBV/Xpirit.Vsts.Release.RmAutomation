...
foreach ($blobname in $blobs.Name){
	...
	{
			$items.Add($blobname) | Out-Null
	}
}


$items | Invoke-Parallel -ImportVariables -ImportModules -ScriptBlock {
	$Name = $_.Split('/')[$_.Split('/').Length-1].Split('.')[0].ToString()
		$ModuleURI = -join($SourceContext.BlobEndPoint, $StorageContainerName,"/", $_) 
    
	Write-Output "Deploying module $Name ..."	
    
	New-AzureRmResourceGroupDeployment -Name ((Get-ChildItem $TemplateFile).BaseName + '-'+ $Name + '-' + ((Get-Date).ToUniversalTime()).ToString('MMdd-HHmm')) `
